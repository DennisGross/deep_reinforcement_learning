...
class DeepQNetwork(torch.nn.Module):
    ...
    def __init__(self, input_dimensions, action_dimensions, learning_rate):
        ...
        self.fc1 = torch.nn.Linear(in_features=self.input_dimensions, out_features=256)
        self.fc2 = torch.nn.Linear(in_features=256, out_features=256)
        self.fc3 = torch.nn.Linear(in_features=256, out_features=self.action_dimensions)
...
class DQNAgent:
    ...
    def __init__(self, input_dimensions, action_dimensions, memory_size=10000,
                 epsilon=1, epsilon_dec=0.9996, epsilon_end=0.05, learning_rate=0.01, gamma=0.99, batch_size=64, model_name="model"):
        # Environment related
        self.input_dimensions = input_dimensions
        self.action_dimensions = action_dimensions
        # Hyperparameter
    ...
    def remember(self, state, action, reward, new_state, terminal):
        action_space = [0.0] * self.action_dimensions
        action_space[action] = 1.0
    ...
    def learn(self):
        if self.experience_replay.contains_more_elements_than_batch_size(self.batch_size):
            ...
            Q_s_a_next = self.neural_network.forward(new_state_batch)
            batch_index = torch.arange(self.batch_size)
            y[batch_index, action_indices] = reward_batch \
                                   + self.gamma * torch.max(Q_s_a_next, dim=1)[0] \
                                   * terminal_batch
